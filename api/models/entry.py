...
Validation = Annotated[str, StringConstraints(min_length=1, strip_whitespace=True, max_length=256)]
...
class EntryUpdate(BaseModel):
    """Model for updating a journal entry (PATCH request).

    All fields are optional. When provided, must follow the same validation rules as EntryCreate.
    """

    work: Validation | None = None
    struggle: Validation | None = None
    intention: Validation | None = None
...
class Entry(BaseModel):
    ...
    work: Validation = Field(..., max_length=256, description="What did you work on today?")
    struggle: Validation = Field(
